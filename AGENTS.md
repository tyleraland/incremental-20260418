...
- **Asset pipeline** (authoring guide: `src/render/CLAUDE.md` — read it before touching visuals): colors are named roles in `src/render/palette.ts` (no hex at use sites; no filters/gradients anywhere — `Palette.test.tsx` gates it), seeded geometry helpers in `src/render/authoring.ts`, prop assets as DATA in `src/render/props.ts` (`TERRAIN_PROPS`, `cutout()` for the auto two-tone). Tooling: `?workshop=1` (dev-only live prop editor — JSON in, every-biome/size/jitter preview out, copy-paste snippet) and `npm run import-svg -- file.svg` (normalize an Inkscape/Figma SVG: flatten transforms, fit to the unit box, snap colors to roles, reject filters/gradients).
