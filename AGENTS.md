...
- Game seam: `Location.mapGen = { recipe, seed? }` → `createOpenBattleFor` takes barriers+size from the spec (`generateForLocationCached` — pure gen + static params, session-cached; `specBarriers` drops materials for the engine). Live maps pin `maxBarriers` 16 (the benched pathing envelope; gated in `map-perf-envelope.test.ts`).
- **Render consumption (phase 2)**: `terrain.tsx` takes an optional `spec` — surface plane → organic material washes (`maskLoops` in `authoring.ts`; shallow-under-deep water bands, shoreline stroke), scatter plane → biome prop archetypes (`KIND_ARCHETYPE`, kinds never ids), materials → deep-water rects vanish under the lake, hedges paint foliage. Wash color roles live in `palette.ts` (`meadowWash`/`sandWash`/`waterShallow`/`waterDeep`). First live generated location: **`mirror-vale`** (96×96, south of Kanto Beach). Remaining phases: BACKLOG → *Procedural map generation*.
