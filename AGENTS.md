...
- **Cadence**: store ticks `TICKS_PER_SECOND`=5; one engine round per tick (`ROUND_EVERY_TICKS`=1). Battles run `timeScale`=`ROUND_TIME_SCALE`=6 (fine rounds); open-world fields with `openWorldCap` ≥ 90 drop to a pace-invariant `timeScale 3` every 2 ticks (`openWorldTimeScale`×`everyTicksFor`, product = 6 ⇒ constant cells/sec — see BACKLOG → cadence tiers). `timeScale` defaults to **1** on `BattleState` (engine suite/replays byte-identical) and is applied via the `engine/timescale.ts` ambient.
