...
// confirm. Location-first ("Deploy Heroes" on a location scope) opens the HERO
// picker, destination defaulted to that site but switchable inline; hero-first
// ("Move" on a hero/party scope) opens the DESTINATION picker directly. Bottom
// sheets, so the battlefield stays visible behind the decision.
...
// Region-grouped, tap-to-pick location list — shared by the hero-first
// destination step and the location-first destination switcher.
function LocationList({ locations, units, selectedId, onPick }: {
  locations: Location[]; units: Unit[]; selectedId: string | null; onPick: (id: string) => void
}) {
...
// ── Location-first: pick heroes to deploy, destination defaults to the site
//    that opened the sheet but is switchable ────────────────────────────────--
function HeroPicker({ locId }: { locId: string }) {
  ...
  const [destination, setDestination] = useState(locId)
  const [pickingDest, setPickingDest] = useState(false)
  const [picked, setPicked] = useState<string[]>([])
  const loc = locations.find((l) => l.id === destination)
  if (!loc) return null
  ...
  const confirm = () => {
    ...
    assignUnits(picked, destination)
    if (loc.region !== useGameStore.getState().mapPageId) useGameStore.getState().setMapPage(loc.region)
    useGameStore.setState({ selectedUnitIds: picked, selectedLocationId: destination })
  ...
  return (
    <Sheet
      ...
      sub={pickingDest ? 'pick a destination' : 'pick who goes — they hunt together on arrival'}
      onClose={close}
      footer={
        <button
          onClick={confirm}
          disabled={picked.length === 0 || pickingDest}
          className={['w-full text-sm font-semibold px-3 py-2.5 rounded-lg border transition-colors',
            picked.length && !pickingDest ? 'border-game-primary/70 bg-game-primary/20 text-game-text hover:bg-game-primary/30'
    ...
    >
      <button
        ...
        <span className="text-xs text-game-text flex-1 truncate text-right">{loc.name}</span>
        <span className="text-game-text-dim text-xs shrink-0">{pickingDest ? '▾' : '▸'}</span>
      </button>

      {pickingDest ? (
        <LocationList locations={locations} units={units} selectedId={destination} onPick={chooseDestination} />
      ) : (
        <>
          {here.length > 0 && (
            <div>
...
function LocationPicker({ unitIds }: { unitIds: string[] }) {
  ...
  return (
    ...
    >
      <LocationList locations={locations} units={units} selectedId={picked} onPick={(id) => setPicked((p) => (p === id ? null : id))} />
    </Sheet>
  )
}
