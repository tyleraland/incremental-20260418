...
test('hero tab, monster inspect overlay, skill tree button', async ({ page }, testInfo) => {
  const proj = testInfo.project.name
  const shot = (name: string) => page.screenshot({ path: `e2e/__shots__/u3-${proj}-${name}.png` })
  ...
  await page.waitForTimeout(1500)
  await shot('01-hero-tab')
  await page.getByRole('button', { name: 'Skills', exact: true }).click().catch(() => {})
  await page.waitForTimeout(300)
  await shot('02-skills-button')
  // Tap a monster on the battlefield → inspect overlay covers the lens tabs.
  await page.getByTitle(/Slime —/).first().click().catch(() => {})
  await page.waitForTimeout(500)
  await shot('03-monster-inspect')
})
