...
test('hero summary (no traits) + prominent skill tree', async ({ page }, testInfo) => {
  const proj = testInfo.project.name
  const shot = (name: string) => page.screenshot({ path: `e2e/__shots__/hero-${proj}-${name}.png` })
