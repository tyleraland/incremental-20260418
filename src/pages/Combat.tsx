...
function BigUnitCard({ unit, locationId }: { unit: Unit; locationId: string }) {
  ...
  return (
    <div className="rounded-lg border border-game-border bg-game-surface px-2.5 py-2 space-y-1.5">
...
function BigMonsterCard({ slotIndex, locationId, onOpenCodex }: {
  slotIndex: number; locationId: string; onOpenCodex: (monsterId: string) => void
}) {
  const equipment          = useGameStore((s) => s.equipment)
  const allUnits           = useGameStore((s) => s.units)
  ...
  const borderCls = behavior === 'prioritize' ? 'border-amber-700/60' :
                    behavior === 'avoid'      ? 'border-sky-700/60'   : 'border-game-border'

  return (
    <div className={`rounded-lg border bg-game-bg px-2.5 py-2 space-y-1.5 ${borderCls} ${behavior === 'ignore' ? 'opacity-60' : ''}`}>
      <div className="flex items-center justify-between gap-1">
        <div className="flex items-center gap-1 flex-wrap min-w-0">
        ...
        <button
          onClick={() => onOpenCodex(monster.id)}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-2 min-w-0">
                ...
                {focusedUnits.length > 0 ? (
                  focusedUnits.map((u) => <BigUnitCard key={u.id} unit={u} locationId={focusedLocation.id} />)
                ) : (
