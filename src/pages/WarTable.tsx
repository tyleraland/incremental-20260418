...
import {
  useGameStore, getDerivedStats, getInitials, MONSTER_REGISTRY,
  type Unit, type Location,
...
// deploy bar). Deploy is genuinely wired — select heroes, tap a front, send
// them — so this isn't only a mock; the rest is dressed-up real game state.
//
// It's deliberately its own page (a new 'war' tab) so the existing tabs keep
// working while we experiment. Dirty-but-real per the prototype brief.
// ─────────────────────────────────────────────────────────────────────────────
...
interface FrontInfo {
  allies: number
  foes: number
  live: boolean        // a battle object exists with both sides alive
  round: number
}
function frontInfo(battle: ReturnType<typeof useGameStore.getState>['battles'][string] | undefined): FrontInfo {
...
function Overworld({ locations }: { locations: Location[] }) {
  ...
  const squadSelected = selectedUnitIds.length > 0

  // Only the world page's nodes (the ones we have coords for).
  const nodes = locations.filter((l) => LOCATION_COORDS[l.id])

  // Connection segments, deduped (draw each undirected edge once).
  const edges = useMemo(() => {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden rounded-lg border border-game-border bg-[radial-gradient(ellipse_at_30%_20%,rgba(99,102,241,0.10),transparent_60%),radial-gradient(ellipse_at_80%_90%,rgba(34,211,238,0.08),transparent_55%)] bg-game-bg">
      ...
      {nodes.map((loc) => {
        ...
        const party = units.filter((u) => u.locationId === loc.id)
        const front = frontInfo(battles[loc.id])
        ...
        const fill = live
          ...
          ? 'bg-emerald-950/50'
          : 'bg-game-surface/80'
        const anim = live ? 'animate-war-pulse' : squadSelected && !hasHeroes ? 'animate-war-beacon' : ''

        return (
          <button
            key={loc.id}
            onClick={() => setSelectedLoc(isSel ? null : loc.id)}
            title={loc.name}
          ...
          >
            <span
              className={[
                'relative w-8 h-8 rounded-full border flex items-center justify-center text-base leading-none',
                'transition-all duration-150 group-hover:scale-110',
                ring, fill, anim, isSel ? 'scale-110' : '',
              ].join(' ')}
            >
              <span className="text-game-text-dim drop-shadow">{terrainGlyph(loc.traits)}</span>
              {/* hero count pip */}
              {hasHeroes && (
                <span className="absolute -top-1 -right-1 min-w-[14px] h-[14px] px-0.5 rounded-full bg-game-green text-[9px] font-bold text-game-bg flex items-center justify-center ring-1 ring-game-bg">
                  {party.length}
                </span>
              )}
              {/* live-battle marker */}
              {live && (
                <span className="absolute -bottom-1 -left-1 text-[9px] leading-none">⚔️</span>
              )}
            </span>
            <span
              className={[
                'mt-1 max-w-[72px] truncate text-[9px] leading-tight tracking-wide',
                isSel ? 'text-game-accent' : hasHeroes ? 'text-game-text-dim' : 'text-game-muted',
              ].join(' ')}
            >
              {loc.name}
            </span>
...
function RosterCard({ unit }: { unit: Unit }) {
  ...
  return (
    <button
      ...
      className={[
        'w-full text-left rounded-lg border px-2.5 py-2 transition-all flex items-center gap-2.5',
        selected
    ...
    >
      ...
      <span
        className={[
          'shrink-0 w-9 h-9 rounded-md border flex items-center justify-center text-lg',
...
function Assessment() {
  ...
  const counts = useMemo(() => {
    ...
    for (const u of units) {
      ...
      else c.recovering++  // down or resting
    }
    return c
  }, [units])
  ...
  return (
    <div className="rounded-lg border border-game-border bg-game-surface/60 p-2.5">
      ...
      <div className="mt-2.5">
        ...
        {fronts.length === 0 ? (
          <div className="text-[11px] italic text-game-muted py-1">No squads deployed. Send heroes into the world.</div>
        ) : (
          <div className="flex flex-col gap-1 max-h-28 overflow-y-auto pr-0.5">
            {fronts.map((f) => (
              <div key={f.id} className="flex items-center gap-2 text-[11px] rounded border border-game-border/60 bg-game-bg/40 px-2 py-1">
                ...
                <span className="text-game-muted">{f.heroes}🛡</span>
                {f.live && <span className="text-red-400">{f.foes}👹</span>}
    ...
    <div className="rounded-lg border border-game-border bg-game-surface/90 p-2.5">
      {selected.length === 0 ? (
        <div className="text-[11px] text-game-muted text-center py-1.5">
          Tap heroes to muster a squad, then pick a front to deploy.
        </div>
      ) : (
        <>
          ...
          <div className="flex gap-1.5">
            <button
              disabled={!canDeploy}
              onClick={() => target && assignUnits(selectedUnitIds, target.id)}
              className={[
            ...
            <button
              onClick={() => assignUnits(selectedUnitIds, null)}
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-game-border bg-game-surface/70 shrink-0">
        ...
        {front.live ? (
          <span className="text-[10px] text-red-400 flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" /> ENGAGED · R{front.round}
          </span>
        ) : (
...
export function WarTable() {
  ...
  // Roster ordering: those needing attention float up via status, then level.
  ...
  return (
    <div className="h-full flex flex-col animate-war-rise">
      ...
      {/* ── Body: Theater | Command ──
          Desktop: a fixed two-column split, each side managing its own overflow.
          Mobile: the whole body scrolls, sections fall to comfortable min-heights
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-2 p-2 overflow-y-auto lg:overflow-hidden">
        ...
        <aside className="flex flex-col gap-2 lg:w-[360px] shrink-0 lg:min-h-0">
          ...
          <div className="flex items-center justify-between px-0.5">
            ...
            <span className="text-[10px] text-game-muted">{units.length} heroes</span>
          </div>
