    ...
    <div className="mt-2 rounded-xl border border-game-border bg-game-bg px-4 py-3 space-y-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {totalSlots > 1 && (
            <div className="flex items-center gap-0.5">
              <button
                onClick={() => onSlotIdxChange(Math.max(0, slotIdx - 1))}
                disabled={slotIdx === 0}
                className="w-4 h-4 flex items-center justify-center text-[11px] text-game-text-dim disabled:opacity-30 hover:text-game-text"
              >‹</button>
              <span className="text-[10px] text-game-text-dim tabular-nums">{slotIdx + 1}/{totalSlots}</span>
              <button
                onClick={() => onSlotIdxChange(Math.min(totalSlots - 1, slotIdx + 1))}
      ...
      <div className="flex items-center justify-between text-xs">
        <div className="flex items-center gap-1.5">
          ...
          <span className={`font-medium ${hpColor}`}>{hpPct}%</span>
        </div>
        <span className="text-game-text-dim">{phaseLabel}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <CombatStatCell label="Dealt / sec" value={rollingDps !== null ? rollingDps.toFixed(2) : '—'} />
        <CombatStatCell label="Dealt / min" value={rollingDpm !== null ? Math.round(rollingDpm).toString() : '—'} />
        <CombatStatCell label="Taken / sec" value={rollingTps !== null ? rollingTps.toFixed(2) : '—'} />
        <CombatStatCell label="Taken / min" value={rollingTpm !== null ? Math.round(rollingTpm).toString() : '—'} />
        <CombatStatCell label="Regen / sec" value="0" dim="TODO" />
        <CombatStatCell label="Position"    value={phase !== 'standing' ? `dist. ${distance}` : 'in combat'} />
      </div>
